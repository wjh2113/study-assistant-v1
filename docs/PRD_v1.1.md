...
- 技术基线：Taro 4 + React 18 + NestJS 10 + PostgreSQL 16 + Redis 7
...
| 后端 | NestJS 10 | 模块化单体，便于快速交付和后续拆分 |
| 数据库 | PostgreSQL 16 | 主库首选，兼顾关系查询、统计和 JSONB |
| 缓存/队列 | Redis 7 + BullMQ | 排行榜缓存、异步解析、异步报告 |
| 对象存储 | 阿里云 OSS | 存 PDF、封面、社区图片、TTS 音频 |
| AI 网关 | 服务端统一封装 | 前端不直接调用模型 |
| 向量能力 | pgvector | V1 预留，V1.2 开启 |
| 部署方式 | Docker Compose | 适合现有两台服务器规模 |
...
| 服务器 | 角色 | 部署内容 | 结论 |
|---|---|---|---|
| 京东云 4G/60G | 生产主机 | Nginx、NestJS API、Worker、PostgreSQL、Redis | 作为主生产环境 |
...
- 阿里云可结合 OSS 做对象存储，适合承接上传文件和备份文件
...
| API 服务 | 京东云 | NestJS 主服务 |
| Worker | 京东云 | PDF 解析、排行榜计算、报告生成 |
| PostgreSQL | 京东云 | 主业务数据库 |
| Redis | 京东云 | 缓存与队列 |
| OSS | 阿里云 | 文件存储、备份归档 |
| Staging API | 阿里云 | 预发环境、联调环境 |
| 数据库备份文件 | 阿里云 + OSS | 双份保存 |
...
### 6.6 为什么不选 Oracle / SQL Server

| 方案 | 是否采用 | 原因 |
|---|---|---|
| PostgreSQL 16 | 是 | 与 NestJS、JSONB、统计分析、后续向量扩展最匹配 |
| Oracle | 否 | 成本高、运维重、对当前产品收益不明显 |
| SQL Server | 否 | 更适合 .NET 生态，当前技术栈没有明显优势 |
...
### 10.1 数据库选型结论
- 数据库：PostgreSQL 16
- 扩展：`pgcrypto`、`uuid-ossp`、`pgvector`
- 原因：
  - 关系数据、统计查询和 JSONB 支持强
  - 与 NestJS、Prisma 或 TypeORM 生态兼容
  - 支持未来向量检索
